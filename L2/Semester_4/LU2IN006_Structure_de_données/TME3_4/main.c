...
void mesurer_temps_recherche(char *filename, int n) {
    ...
    // Mesurer le temps de recherche pour la structure de liste
    ...
    // Mesurer le temps de recherche pour la table de hachage
    double temps_table_hachage = 0.0;
    for (int i = 0; i < 10; i++) {
        clock_t debut_table_hachage = clock();
        LivreH *dup_table_hachage = dupliquesH(b);
    ...
    printf("Temps de recherche pour la table de hachage: %f secondes\n", temps_table_hachage);
...
int main(int argc, char** argv){
    ...
    // Lecture progressive des lignes du fichier
    int m = 1000;
    for (m; m <= 50000; m *= 2) {
        printf("Mesure du temps de recherche pour n = %d\n", m);
        mesurer_temps_recherche("GdeBiblio.txt", m);
    }
